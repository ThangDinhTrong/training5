    ...
    */
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.row % 2 == 0 {
            self.performSegueWithIdentifier("navi", sender: self)
        }
        else {
            self.performSegueWithIdentifier("pre", sender: self)
        }
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
            ...
            if let indexPath = tableView.indexPathForSelectedRow {
                let destination = segue.destinationViewController as! T4DetailViewController
                destination.objectAV = avResultArray[indexPath.row]
               // destination.detailImage.image = UIImage(named: "restaurant.jpg")
            }
